...
#canvas = cv2.imread('./data/bengal_blue.jpg')
canvas = cv2.imread('./data/mux_puzzle.jpg')
# Import our piece (we are going to use a clump for now)
#piece = cv2.imread('./data/piece.jpg')
piece = cv2.imread('./data/mux_piece.jpg')

# Pre-process the piece
def identify_contour(piece, threshold_low=100, threshold_high=255):
    """Identify the contour around the piece"""
    piece = cv2.cvtColor(piece, cv2.COLOR_BGR2GRAY) # better in grayscale
    ret, thresh = cv2.threshold(piece, threshold_low, threshold_high, cv2.THRESH_TOZERO)
    image, contours, heirarchy = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    #print(contours) #bounding boxes
    contour_sorted = np.argsort(map(cv2.contourArea, contours))
    #print(contour_sorted) # boxes sorted by area
    #print(contours[0])
    #print(cv2.contourArea(contours[0]))
    cv2.imwrite('./output/piece_processed.jpg', image)
    return contours, contour_sorted[-1]
...
cv2.imwrite('./output/piece_cropped.jpg', cropped_piece)

# Initiate SIFT detector
sift =  cv2.xfeatures2d.SIFT_create()

img1 = cropped_piece.copy() # queryImage
#img1 = cv2.imread('./output/piece_processed.jpg') # try with the high-contrast bitmap, maybe?
